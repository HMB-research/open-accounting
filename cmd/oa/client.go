...
func (c *apiClient) revokeAllAuthSessions(ctx context.Context, accessToken string) error {
	bearerToken := strings.TrimSpace(accessToken)
	if bearerToken == "" {
		bearerToken = c.apiToken
	}
	return c.request(ctx, http.MethodDelete, "/api/v1/auth/sessions", nil, bearerToken, nil)
}
...
func (c *apiClient) listMigrationProviderPresets(ctx context.Context, tenantID string) ([]cutover.MigrationProviderPresetInfo, error) {
	var resp []cutover.MigrationProviderPresetInfo
	if err := c.request(ctx, http.MethodGet, path.Join("/api/v1/tenants", tenantID, "migration", "provider-presets"), nil, c.apiToken, &resp); err != nil {
		return nil, err
	}
	return resp, nil
}
...
func (c *apiClient) issueStock(ctx context.Context, tenantID string, req *inventory.IssueStockRequest) (*inventory.IssueStockResult, error) {
	var resp inventory.IssueStockResult
	if err := c.request(ctx, http.MethodPost, path.Join("/api/v1/tenants", tenantID, "inventory", "issue"), req, c.apiToken, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *apiClient) postJournalEntry(ctx context.Context, tenantID, entryID, reason string) (map[string]string, error) {
	var resp map[string]string
	body := accounting.PostJournalEntryRequest{Reason: strings.TrimSpace(reason)}
	if err := c.request(ctx, http.MethodPost, path.Join("/api/v1/tenants", tenantID, "journal-entries", entryID, "post"), body, c.apiToken, &resp); err != nil {
		return nil, err
	}
	return resp, nil
}
