...
func (a *cliApp) runOpsBackup(ctx context.Context, args []string) error {
	...
	case "schedule-systemd":
		...
		systemdUnitDir := fs.String("systemd-unit-dir", "", "Host systemd unit directory for the generated install helper")
		offsiteProvider := fs.String("offsite-provider", "", "Offsite credential template to generate: s3 or rclone")
		...
		scriptArgs = appendStringFlag(scriptArgs, "systemd-unit-dir", *systemdUnitDir)
		scriptArgs = appendStringFlag(scriptArgs, "offsite-provider", *offsiteProvider)
		scriptArgs = appendStringFlag(scriptArgs, "retention-days", *retentionDays)
		scriptArgs = appendStringFlag(scriptArgs, "max-age-hours", *maxAgeHours)
...
func parseBankAccountCSVRows(content string) ([]banking.CSVBankAccountRow, error) {
	return banking.ParseBankAccountCSVRows(content)
}
