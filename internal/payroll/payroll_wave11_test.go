...
func TestWave11NewServiceUsesInjectedGormDB(t *testing.T) {
	...
	require.IsType(t, &DefaultUUIDGenerator{}, service.uuid)
}
...
func TestWave11NewServicePanicsOnInjectedGormError(t *testing.T) {
	...
	require.PanicsWithError(t, "create payroll GORM repository: pool unavailable", func() {
		_ = NewService(new(pgxpool.Pool))
	})
}

func TestWave11NewAbsenceServiceWithPoolUsesInjectedGormDB(t *testing.T) {
	expectedDB := &gorm.DB{}
	pool := new(pgxpool.Pool)
	...
	require.NotNil(t, service)
	repo, ok := service.repo.(*AbsenceGORMRepository)
	require.True(t, ok)
	require.Same(t, expectedDB, repo.db)
	require.IsType(t, &DefaultUUIDGenerator{}, service.uuid)
	require.Same(t, evidence, service.evidence)
}

func TestWave11NewAbsenceServiceWithPoolPanicsOnInjectedGormError(t *testing.T) {
	stubWave11GormDBFromPool(t, func(context.Context, *pgxpool.Pool) (*gorm.DB, error) {
		return nil, errors.New("pool unavailable")
	})

	require.PanicsWithError(t, "create absence GORM repository: pool unavailable", func() {
		_ = NewAbsenceServiceWithPool(new(pgxpool.Pool))
	})
}
