...
func TestRepository_DepreciationEntry(t *testing.T) {
	...
	entry := &DepreciationEntry{
		...
		Period:                  "2025-01",
		Amount:                  decimal.NewFromFloat(180.00), // (12000-1200)/60 = 180 per month
		AccumulatedDepreciation: decimal.NewFromFloat(180.00),
		BookValue:               decimal.NewFromFloat(11820.00),
		CreatedAt:               time.Now(),
	}
	...
	if !entries[0].Amount.Equal(decimal.NewFromFloat(180.00)) {
		t.Errorf("expected amount 180.00, got %s", entries[0].Amount)
	}
}
