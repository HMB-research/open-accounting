...
func (r *PostgresRepository) ListActiveTenants(ctx context.Context) ([]TenantInfo, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, schema_name, COALESCE(name, ''), COALESCE(settings->>'period_lock_date', '') FROM tenants WHERE is_active = true
	`)
	if err != nil {
	...
	for rows.Next() {
		var t TenantInfo
		if err := rows.Scan(&t.ID, &t.SchemaName, &t.CompanyName, &t.PeriodLockDate); err != nil {
