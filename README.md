...
- [ ] Remaining mutating migration cutover orchestration beyond the guarded CLI/server-side execution, resume-snapshot, saved-run list/get/event-stream paths, provider preset catalog discovery, dashboard workbench live streaming, saved-run progress/duration telemetry, and accountant-workspace launch handoff, especially additional provider-specific cutover presets and deeper dashboard-side cutover controls
