...
- [x] Dashboard migration workbench for cutover bundle assembly, validation, execution planning, saved dry runs, confirmed execution, saved-run monitoring, progress/active-step/duration telemetry, and resume-by-ID selection
...
- [ ] Remaining mutating migration cutover orchestration beyond the guarded CLI/server-side execution, resume-snapshot, saved-run list/get paths, dashboard workbench, and saved-run progress/duration telemetry, especially accountant-workspace launch handoff, live streaming telemetry, and vendor-specific cutover presets
- [ ] Remaining tax/payroll edges and deeper accountant-workspace execution beyond the current assignment queue, direct bank follow-up, reminder actions, pending-document assignment approval, payroll-run payment-date setting and approval, payroll TSD assignment generation, KMD assignment regeneration/export, expense submit/approve/post assignment completion, fiscal-year close/carry-forward assignment completion, and the standalone migration workbench, especially remaining document evidence/policy follow-up
