...
>
> Full local baseline last verified on 2026-06-08. On 2026-06-11, local revalidation covered `make test-cli-coverage`, `go test -timeout=3m ./docs -count=1`, and recent frontend stage gates including `cd frontend && bun run test:prepared` with 30 files and 546 tests; PR #62 CI run `27364049917` revalidated `golangci-lint run`, `DATABASE_URL=postgres://openaccounting:openaccounting@localhost:5432/openaccounting?sslmode=disable make test-integration-coverage`, frontend lint/check/test/build, smoke E2E, and all four seeded demo E2E shards.
