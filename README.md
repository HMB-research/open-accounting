...
- [x] Dashboard migration workbench for cutover bundle assembly, validation, execution planning, saved dry runs, confirmed execution, saved-run monitoring, progress/active-step/duration telemetry, saved-run event stream API/CLI access, resume-by-ID selection, and accountant-workspace assignment handoff into deep-linked saved runs
...
- [ ] Remaining mutating migration cutover orchestration beyond the guarded CLI/server-side execution, resume-snapshot, saved-run list/get/event-stream paths, dashboard workbench, saved-run progress/duration telemetry, and accountant-workspace launch handoff, especially dashboard-side live stream wiring and vendor-specific cutover presets
