...
| **Migration Preflight**        | Non-mutating CSV/XML bundle validation for required columns, duplicate identifiers, account, contact, employee, payroll-history, commercial-document, inventory, fixed-asset, cost-center/allocation, expense, payment, bank-account, and bank-transaction row values, grouped document consistency, and cross-file references before cutover imports, including e-invoice XML, expenses, commercial history, inventory, banking, tax, cost allocations, and fixed assets |
