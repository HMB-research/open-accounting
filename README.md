...
- [x] Guarded CLI and server-side migration execution that validates, plans, requires explicit confirmation, and runs ready cutover import steps through the existing tenant-scoped APIs and import services
...
- [ ] Remaining mutating migration cutover orchestration beyond the guarded CLI/server-side execution paths, especially durable dashboard execution, resumability, and richer vendor-specific cutover presets
- [ ] Remaining tax/payroll edges and deeper accountant-workspace execution beyond the current assignment queue, direct bank follow-up, reminder actions, pending-document assignment approval, payroll-run payment-date setting and approval, payroll TSD assignment generation, KMD assignment regeneration/export, expense submit/approve/post assignment completion, and fiscal-year close/carry-forward assignment completion, especially dashboard-initiated migration execution and remaining document evidence/policy follow-up
