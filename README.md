...
- [x] Guarded CLI migration execution that validates, plans, requires `--confirm`, and runs ready cutover import steps through the existing tenant-scoped APIs
...
- [ ] Remaining mutating migration cutover orchestration beyond the guarded CLI execution path, especially durable server-side/dashboard execution, resumability, and richer vendor-specific cutover presets
