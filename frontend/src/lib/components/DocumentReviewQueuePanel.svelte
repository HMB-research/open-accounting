<script lang="ts">
  ...
  const entityTypeOptions: EntityTypeFilter[] = [
    "",
    "invoice",
    "journal_entry",
    "payment",
  ...
  async function loadReviewQueue() {
    ...
    try {
      ...
      error =
        err instanceof Error ? err.message : m.documents_loadReviewQueueError();
    } finally {
      isLoading = false;
    }
  }
  ...
  async function loadRetentionReview() {
    ...
    try {
      ...
      error =
        err instanceof Error ? err.message : m.documents_loadRetentionError();
    } finally {
      isLoading = false;
    }
  }
  ...
  function syncReviewNotes(documents: DocumentAttachment[]) {
    reviewNotes = Object.fromEntries(
      documents.map((doc) => [
        doc.id,
        reviewNotes[doc.id] ?? doc.review_note ?? "",
      ]),
    );
  }

  function syncRetentionEdits(documents: DocumentAttachment[]) {
    retentionEdits = Object.fromEntries(
      documents.map((doc) => [
        doc.id,
        retentionEdits[doc.id] ?? dateInputValue(doc.retention_until),
      ]),
    );
  }
  ...
  async function updateRetention(
    ...
    error = "";
    try {
      await api.updateDocumentRetention(
        tenantId,
        doc.id,
        clearRetention
      ...
      error =
        err instanceof Error ? err.message : m.documents_retentionUpdateError();
    } finally {
      mutatingDocumentId = "";
    }
  }
  ...
  function formatEntityType(value: DocumentAttachment["entity_type"]): string {
    switch (value) {
      ...
      case "expense":
        return m.documents_entityExpense();
      case "quote":
        return m.documents_entityQuote();
      case "order":
        return m.documents_entityOrder();
      case "leave_record":
  ...
  function formatDocumentType(
    value: DocumentAttachment["document_type"],
  ): string {
    switch (value) {
  ...
  function formatReviewStatus(
    value: DocumentAttachment["review_status"],
  ): string {
    switch (value) {
      case "REVIEWED":
...
<div class="document-review-panel">
  ...
  <div
    class="queue-tabs"
    role="tablist"
    aria-label={m.documents_reviewQueueTabsLabel()}
  >
    <button
      type="button"
      role="tab"
    ...
    <section class="filter-band" aria-label={m.documents_reviewQueueFilters()}>
      {#if activeView === "review"}
        <div class="filter-grid">
          <div class="form-group">
            <label class="label" for="document-review-status"
              >{m.documents_reviewStatusLabel()}</label
            >
            <select
            ...
            >
              {#each reviewStatusOptions as option (option)}
                <option value={option}
                  >{option === "ALL"
                    ? m.documents_statusAll()
                    : formatReviewStatus(option)}</option
                >
              {/each}
            </select>
          </div>
          <div class="form-group">
            <label class="label" for="document-review-entity"
              >{m.documents_entityTypeLabel()}</label
            >
            <select
              id="document-review-entity"
              class="input"
              ...
              {#each entityTypeOptions as option (option)}
                <option value={option}
                  >{option
                    ...
                    : m.documents_entityTypeAll()}</option
                >
              {/each}
            </select>
          </div>
          <div class="form-group">
            <label class="label" for="document-review-type"
              >{m.documents_typeLabel()}</label
            >
            <select
              ...
              {#each documentTypeOptions as option (option)}
                <option value={option}
                  >{option
                    ...
                    : m.documents_documentTypeAll()}</option
                >
              {/each}
            </select>
          </div>
          <div class="form-group compact">
            <label class="label" for="document-review-limit"
              >{m.documents_limitLabel()}</label
            >
            <select
      ...
      {:else}
        <div class="filter-grid retention">
          <div class="form-group">
            <label class="label" for="retention-as-of"
              >{m.documents_asOfDate()}</label
            >
            <input
              id="retention-as-of"
              class="input"
              type="date"
              bind:value={retentionAsOf}
            />
          </div>
          <div class="form-group compact">
            <label class="label" for="retention-horizon"
              >{m.documents_horizonDays()}</label
            >
            <input
              ...
              min="0"
              step="1"
              bind:value={retentionHorizonDays}
            />
          </div>
          <label class="checkbox-row" for="include-missing-retention">
            <input
              id="include-missing-retention"
              type="checkbox"
              bind:checked={includeMissingRetention}
            />
    ...
    {:else}
      ...
      {#if activeDocuments.length === 0}
        <div class="card empty-state">
          <p>
            {activeView === "review"
              ? m.documents_reviewQueueEmpty()
              : m.documents_retentionQueueEmpty()}
          </p>
        </div>
      {:else}
                ...
                <tr>
                  <td data-label={m.documents_reviewQueueFile()}>
                    <div class="document-file">
                      ...
                      <span
                        >{formatDocumentType(doc.document_type)} · {formatFileSize(
                          doc.file_size,
                        )}</span
                      >
                      {#if doc.notes}
                  ...
                  <td data-label={m.common_status()}>
                    <div class="status-stack">
                      <span class={getStatusClass(doc.review_status)}
                        >{formatReviewStatus(doc.review_status)}</span
                      >
                      {#if doc.reviewed_at}
                        <span
                          >{m.documents_reviewedAt({
                  ...
                  <td data-label={m.documents_retentionUntil()}
                    >{formatDate(doc.retention_until)}</td
                  >
                  <td data-label={m.documents_reviewQueueUploaded()}
                    >{formatDateTime(doc.created_at)}</td
                  >
                  <td data-label={m.common_actions()} class="actions-cell">
                    <div class="review-actions">
                      {#if activeView === "retention"}
                        <div class="retention-editor">
                          <label class="label" for={`retention-date-${doc.id}`}
                            >{m.documents_retentionDateFor({
                              file: doc.file_name,
                            })}</label
                          >
                          <div class="retention-row">
                            ...
                            <button
                              type="button"
                              class="btn btn-secondary"
                              onclick={() => updateRetention(doc, true)}
                              disabled={mutatingDocumentId === doc.id}
                            >
                      ...
                      {/if}
                      <label class="label" for={`queue-review-note-${doc.id}`}
                        >{m.documents_reviewNoteFor({
                          file: doc.file_name,
                        })}</label
                      >
                      <textarea
                      ...
                      <div class="action-row">
                        <button
                          ...
                          onclick={() => applyReviewDecision(doc, "APPROVED")}
                          disabled={mutatingDocumentId === doc.id}
                        >
                          {m.documents_approve()}
                        </button>
                        ...
                        <button
                          ...
                          onclick={() => applyReviewDecision(doc, "REJECTED")}
                          disabled={mutatingDocumentId === doc.id}
                        >
                          {m.documents_reject()}
                        </button>
