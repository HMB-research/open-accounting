<script lang="ts">
  /**
   * Modal dialog for creating and editing contacts (customers/suppliers).
  ...
  interface Props {
    ...
    open: boolean;
    /** ID of the tenant to create or update the contact for */
    tenantId: string;
    /** Existing contact to edit. Omit for create mode. */
    contact?: Contact | null;
    /** Callback when contact is successfully created or updated */
    onSave: (contact: Contact) => void;
    /** Callback when modal is closed (cancel or backdrop click) */
    onClose: () => void;
  }

  let { open, tenantId, contact = null, onSave, onClose }: Props = $props();
  ...
  let isEditMode = $derived(Boolean(contact));
  ...
  $effect(() => {
    ...
    if (activeFormKey === formKey) return;
  ...
  function populateForm(contact: Contact) {
    ...
    newType = contact.contact_type;
    newEmail = contact.email || "";
    newPhone = contact.phone || "";
    newVatNumber = contact.vat_number || "";
    newAddress = contact.address_line1 || "";
    newCity = contact.city || "";
    newPostalCode = contact.postal_code || "";
    newCountry = contact.country_code || "EE";
    newPaymentDays = contact.payment_terms_days || 14;
    error = "";
  }
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      const savedContact = contact
        ? await api.updateContact(tenantId, contact.id, contactPayload)
        : await api.createContact(tenantId, {
            ...contactPayload,
...
{#if open}
  ...
  <div class="modal-backdrop" onclick={handleClose} role="presentation">
    <div
      ...
      tabindex="-1"
    >
      <h2 id="create-contact-title">
        {isEditMode ? m.contacts_editContact() : m.contacts_newContact()}
      </h2>
      ...
      <form onsubmit={handleSubmit}>
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            {#if isEditMode}
              <p class="form-hint">{m.contacts_typeReadOnlyHint()}</p>
            {/if}
          </div>
        </div>
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label class="label" for="contact-postal"
              >{m.contacts_postalCode()}</label
            >
            <input
              class="input"
              type="text"
          ...
          <div class="form-group">
            <label class="label" for="contact-country"
              >{m.contacts_country()}</label
            >
        ...
        <div class="form-group">
          <label class="label" for="contact-payment-days"
            >{m.contacts_paymentTermsDays()}</label
          >
          <input
            class="input"
            type="number"
