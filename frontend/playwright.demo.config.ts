...
const configuredGlobalTimeout = Number(
  process.env.PLAYWRIGHT_GLOBAL_TIMEOUT_MS || "0",
);
const localBaseURL = new URL(baseURL);
const localWebServerHost = localBaseURL.hostname;
const localWebServerPort =
  localBaseURL.port || (localBaseURL.protocol === "https:" ? "443" : "80");
...
export default defineConfig({
  ...
  globalTimeout:
    configuredGlobalTimeout > 0 ? configuredGlobalTimeout : undefined,
  reporter: [
