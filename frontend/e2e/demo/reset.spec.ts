...
// Reset tests mutate shared demo tenants and must run in isolation.
// Keep them out of the default local/CI matrix to avoid racing with CRUD specs.
const DEMO_SECRET = process.env.DEMO_RESET_SECRET;
const IS_CI = !!process.env.CI;
const RUN_DEMO_RESET_TESTS = process.env.RUN_DEMO_RESET_TESTS === "true";
test.describe("Demo Data Reset Verification", () => {
  test.skip(
    !DEMO_SECRET || IS_CI || !RUN_DEMO_RESET_TESTS,
    "Skipped: run reset.spec.ts in isolation with DEMO_RESET_SECRET and RUN_DEMO_RESET_TESTS=true",
  );
  ...
  test.describe("Initial State Verification", () => {
    ...
    test("has correct contact count and key contacts", async ({}, testInfo) => {
      ...
      expect(status.contacts.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.contacts.count,
      );
    ...
    test("has correct journal entry count and key entries", async ({}, testInfo) => {
      ...
      expect(status.journalEntries.count).toBe(
        EXPECTED_DEMO_DATA.journalEntries.count,
      );
      expect(status.journalEntries.keys).toContain(
        getExpectedJournalEntryKey(userNum),
      );
    });
    ...
    test("has correct bank account count", async ({}, testInfo) => {
      ...
      expect(status.bankAccounts.count).toBe(
        EXPECTED_DEMO_DATA.bankAccounts.count,
      );
    ...
    test("has correct recurring invoice count", async ({}, testInfo) => {
      ...
      expect(status.recurringInvoices.count).toBe(
        EXPECTED_DEMO_DATA.recurringInvoices.count,
      );
    ...
    test("has correct payroll run count", async ({}, testInfo) => {
      ...
      expect(status.payrollRuns.count).toBe(
        EXPECTED_DEMO_DATA.payrollRuns.count,
      );
    ...
    test("has correct TSD declaration count", async ({}, testInfo) => {
      ...
      expect(status.tsdDeclarations.count).toBe(
        EXPECTED_DEMO_DATA.tsdDeclarations.count,
      );
  ...
  test.describe("Reset Functionality", () => {
    test("reset is idempotent", async ({}, testInfo) => {
      ...
      expect(statusAfterSecond.accounts.count).toBe(
        statusAfterFirst.accounts.count,
      );
      expect(statusAfterSecond.contacts.count).toBe(
        statusAfterFirst.contacts.count,
      );
      expect(statusAfterSecond.invoices.count).toBe(
        statusAfterFirst.invoices.count,
      );
      expect(statusAfterSecond.employees.count).toBe(
        statusAfterFirst.employees.count,
      );
    });
    ...
    test("reset restores expected counts", async ({}, testInfo) => {
      ...
      expect(status.accounts.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.accounts.count,
      );
      expect(status.contacts.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.contacts.count,
      );
      expect(status.invoices.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.invoices.count,
      );
      expect(status.employees.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.employees.count,
      );
      expect(status.payments.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.payments.count,
      );
      expect(status.journalEntries.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.journalEntries.count,
      );
      expect(status.bankAccounts.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.bankAccounts.count,
      );
      expect(status.recurringInvoices.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.recurringInvoices.count,
      );
      expect(status.payrollRuns.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.payrollRuns.count,
      );
      expect(status.tsdDeclarations.count).toBeGreaterThanOrEqual(
        EXPECTED_DEMO_DATA.tsdDeclarations.count,
      );
    });
  });
