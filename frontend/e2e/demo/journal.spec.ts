...
test.describe('Demo Journal Entries - Page Structure Verification', () => {
	...
	test('displays journal page heading', async ({ page }) => {
		await expect(page.getByRole('heading', { name: /journal|ledger|entries/i })).toBeVisible();
	});

	test('shows create entry button', async ({ page }) => {
		await expect(page.getByRole('button', { name: /new|create|add/i })).toBeVisible();
	});

	test('loads journal data or shows empty state', async ({ page }) => {
		await page.waitForTimeout(5000);
		const hasData = await page.locator('table tbody tr').count() > 0;
		const hasEmptyState = await page.getByText(/no entries|no data|empty|loading/i).isVisible().catch(() => false);
