...
import { ensureAuthenticated, ensureDemoTenant, navigateTo } from '../demo/utils';
...
async function expectVisible(page: Page, selectors: string[]) {
	...
	throw new Error(`Expected one of these selectors to be visible: ${selectors.join(', ')}`);
}
...
test.describe('Smoke - Core Accountant Flow', () => {
	...
	test('dashboard loads for an authenticated tenant', async ({ page }, testInfo) => {
		await navigateTo(page, '/dashboard', testInfo);

		await expect(page).toHaveURL(/dashboard/);
		await expectVisible(page, ['main h1', '.dashboard-header', '[data-testid="dashboard"]']);
	});

	test('invoices page exposes the primary create action', async ({ page }, testInfo) => {
	...
	test('banking page renders account and transaction views', async ({ page }, testInfo) => {
		await navigateTo(page, '/banking', testInfo);

		await expect(page.getByRole('heading', { level: 1 })).toBeVisible({ timeout: 10000 });
		await expectVisible(page, ['table tbody tr', '.bank-account-card', '.empty-state']);
	});

	test('payroll page renders payroll workflow controls', async ({ page }, testInfo) => {
