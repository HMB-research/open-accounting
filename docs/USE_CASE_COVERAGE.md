...
| Plugins, webhooks, and external integrations | `Partial` | Plugin registry/install/permission/enablement/settings flows, loopback-only out-of-process HTTP backend runtime for manifest-declared hooks and tenant-scoped plugin routes, declarative frontend slot cards/links/actions with deterministic ordering and internal-route validation, webhook subscriptions, signed delivery, test delivery, and delivery history. | Backend manifest/runtime tests, focused plugin runtime API tests, frontend plugin manager and slot component tests, API docs, CLI docs, and status docs. | Dynamic frontend component loading, runtime process supervision/packaging, and broader production sandboxing remain incomplete. |
...
4. Add deeper executable accountant workspace exception actions beyond confirmation-ready migration saved-run execution, document-retention date setting, evidence upload/replacement, unapproved-evidence approval, KMD INF/EU VAT OSS report generation, and TSD/KMD acceptance marking, plus remaining payroll/document/evidence-policy edges.
...
7. Harden plugin runtime process supervision/packaging and add dynamic frontend component loading beyond the current loopback HTTP backend runtime and declarative slot runtime.
