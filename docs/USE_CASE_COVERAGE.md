...
- PR #62 on `feat/payroll-history-import` was green at commit `c5cd5bc` in CI run `27474353861`.
...
| Close, reopen, year-end, and carry-forward controls | `Partial` | Period close/reopen, audit history, fiscal-year reviewer sign-off, close packs, approved close-pack evidence, fiscal-year inventory costing review, machine-readable remediation actions for period-close, close-pack evidence, retained earnings, inventory costing, already-posted carry-forward, and carry-forward posting with workspace assignment metadata, ZIP export, carry-forward posting, and carry-forward reversal. | Backend tests, focused accounting/API/CLI close remediation tests, generated OpenAPI docs, CLI docs, frontend API type checks, and status docs. | Remediation actions still need to extend into executable accountant-workspace assignment surfaces. |
| Accountant review workspace | `Partial` | Tenant review queue, cross-tenant portfolio rollup, overdue invoice actions, bank follow-up states with remediation actions, journal evidence follow-up, document-review links, and close, migration preflight, KMD, TSD declaration, payroll run, expense claim, banking, and document remediation data available through API/CLI for workspace integration with workspace queues, stable assignment keys, priorities, and due windows. | Frontend/backend tests, smoke E2E, seeded demo E2E, focused close/migration/KMD/TSD/payroll/banking/expense/document remediation tests, and status docs. | Deeper executable workspace assignment across tax, payroll, documents, migration execution, banking follow-up, expense follow-up, and close remediation remains missing. |
...
4. Add deeper executable accountant workspace exception actions for tax, payroll, close, document, migration execution, banking, and expense follow-up.
