...
| Inventory and warehouses | `Partial` | Product/category/warehouse CRUD, imports, stock adjustments, stock import with lot metadata, serialized stock import guards, warehouse stock levels, cost-preserving lot/serial/expiry transfers with source-lot quantity validation, lot-aware reservation allocation and release, lot-aware issue allocation with lot, weighted-average, or standard-cost issue costing plus accounting-ready or transactionally posted COGS journal lines, pick lists, lot reports, and standard-cost/weighted-average/FIFO valuation. | Backend tests, integration gates, API docs, CLI docs, migration tests, and migration validator tests. | Deeper period-close costing controls remain thin. |
...
1. Extend inventory costing beyond posted issue accounting and issue-level costing methods with deeper period-close costing controls.
