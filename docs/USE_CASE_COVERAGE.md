...
- The follow-up plugin package runtime automatic restart stage was locally revalidated with `go test ./internal/plugin -run TestService_PackageRuntimeAutomaticallyRestartsAfterCrash -count=1`.
...
- The follow-up document retention purge automation stage was locally revalidated with focused documents/API/CLI tests, Swagger regeneration, docs status tests, and the CLI coverage gate.
...
- The follow-up failed-login audit and throttling stage was locally revalidated with focused auth limiter and API login failure tests.
- The follow-up instance-admin route hardening and stock-adjustment generated-ID preflight stage was locally revalidated with focused API admin-route tests, cutover validator tests, docs status tests, lint, and the CLI coverage gate.
...
Accountant workspace coverage now also includes TSD/KMD tax-support evidence
assignment discovery from document evidence-policy checks plus direct
declaration submission actions for TSD e-MTA references and KMD review rows.
Leave-record approval evidence conflicts now return document evidence-policy
...
field. Commercial-document import execution now resolves invoice, quote, order,
...
KMD history preflight now reconciles declared output/input VAT totals against
same-bundle invoice, e-invoice, and VAT-rated historical journal support totals
before execution.
...
2. Complete the remaining historical cutover mutating orchestration polish, further provider-specific mapping depth, cross-file validation outside payroll/TSD history and KMD VAT support, and deeper dashboard cutover controls.
