...
| Inventory and warehouses | `Partial` | Product/category/warehouse CRUD, imports, stock adjustments, stock import with lot metadata, serialized stock import guards, warehouse stock levels, cost-preserving lot/serial/expiry transfers with source-lot quantity validation, lot-aware reservation allocation and release, lot-aware issue allocation with accounting-ready COGS lines, pick lists, lot reports, and standard-cost/weighted-average/FIFO valuation. | Backend tests, integration gates, API docs, CLI docs, migration tests, and migration validator tests. | Automated GL posting for inventory issues and deeper costing policy controls remain thin. |
...
1. Deepen inventory issue accounting beyond costed issue allocations into posted GL automation and configurable costing policies.
