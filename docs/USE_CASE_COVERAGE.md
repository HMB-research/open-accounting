...
| Document attachments, retention, and evidence policy | `Partial` | Upload/list/download/delete/review/approve/reject, retention metadata, review queues, retention review, retention reminder actions, evidence policy checks, and workflow blockers for reconciliation, assets, purchase invoices, journal entries, payments, expenses, leave records, and close packs. | Backend tests, API docs, CLI docs, and docs status checks. | Broader workflow-level policy enforcement and scheduled reminder delivery remain incomplete. |
...
2. Expand document policy enforcement and scheduled retention reminder delivery beyond the current workflow blockers.
