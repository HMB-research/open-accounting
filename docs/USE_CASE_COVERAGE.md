...
| Plugins, webhooks, and external integrations | `Partial` | Plugin registry/install/permission/enablement/settings flows, loopback-only out-of-process HTTP backend runtime and supervised package runtime startup/proxy/shutdown for manifest-declared hooks and tenant-scoped plugin routes, package runtime lifecycle/health/crash/backoff status plus manual restart through API/CLI, automatic package runtime restart after unexpected exits, declarative frontend slot cards/links/actions, safe operator-bundled frontend component registration, deterministic slot ordering, internal-route validation, webhook subscriptions, signed delivery, test delivery, and delivery history. | Backend manifest/runtime tests, focused plugin runtime API tests including forced-crash automatic restart coverage, frontend plugin manager and slot component tests, API docs, CLI docs, and status docs. | OS-level sandboxing and broader production plugin isolation remain incomplete. |
...
5. Finish remaining auth hardening beyond failed-login audit/throttling and live operational rollout proof for accounting-firm pilots.
6. Harden provider-specific backup credential provisioning and host-specific timer enablement.
7. Harden plugins beyond the current loopback HTTP runtime, supervised package runtime startup/proxy/shutdown/status/manual restart/automatic crash restart, and safe operator-bundled frontend component registry, especially OS-level sandboxing and broader production isolation.
