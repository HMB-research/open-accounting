...
| Document attachments, retention, and evidence policy | `Partial` | Upload/list/download/delete/review/approve/reject, retention metadata, review queues, retention review, retention reminder actions, scheduled retention reminder digest delivery, evidence policy checks, and workflow blockers for reconciliation, assets, purchase invoices, journal entries, payments, expenses, leave records, and close packs. | Backend tests, scheduler tests, API docs, CLI docs, and docs status checks. | Broader workflow-level policy enforcement plus reminder retry/escalation controls remain incomplete. |
...
2. Expand document policy enforcement plus retention reminder retry/escalation controls beyond the current workflow blockers and scheduled digest delivery.
