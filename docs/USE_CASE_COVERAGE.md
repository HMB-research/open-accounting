...
| Plugins, webhooks, and external integrations | `Partial` | Plugin registry/install/permission/enablement/settings flows, loopback-only out-of-process HTTP backend runtime and supervised package runtime startup/proxy/shutdown for manifest-declared hooks and tenant-scoped plugin routes, package runtime lifecycle/health/crash/backoff status plus manual restart through API/CLI, declarative frontend slot cards/links/actions, safe operator-bundled frontend component registration, deterministic slot ordering, internal-route validation, webhook subscriptions, signed delivery, test delivery, and delivery history. | Backend manifest/runtime tests, focused plugin runtime API tests, frontend plugin manager and slot component tests, API docs, CLI docs, and status docs. | Automatic restart policy, OS-level sandboxing, and broader production plugin isolation remain incomplete. |
...
7. Harden plugins beyond the current loopback HTTP runtime, supervised package runtime startup/proxy/shutdown/status/manual restart, and safe operator-bundled frontend component registry, especially automatic restart policy, OS-level sandboxing, and broader production isolation.
