...
| Plugins, webhooks, and external integrations | `Partial` | Plugin registry/install/permission/enablement/settings flows, declarative frontend slot cards/links/actions with deterministic ordering and internal-route validation, webhook subscriptions, signed delivery, test delivery, and delivery history. | Backend manifest tests, frontend plugin manager and slot component tests, API docs, CLI docs, and status docs. | Plugin backend route/hook runtime execution and dynamic frontend component loading remain incomplete. |
...
7. Add a sandboxed plugin backend route/hook runtime and dynamic frontend component loading beyond the current declarative slot runtime.
