...
| CLI and API token automation | `Working` | `cmd/oa` supports token bootstrap, token management, accounts, contacts, employees, payroll run lifecycle, payroll history, leave balances, TSD/KMD declarations and exports, invoices, payments, core reports, document workflows, and opening-balance imports using tenant-scoped API tokens. |
