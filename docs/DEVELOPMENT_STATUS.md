...
| Quotes, orders, fixed assets | `Working` | Features exist and have tests, including fixed-asset CSV import for cutover numbers, status, and depreciation/book values. Accountant-grade polish is still limited. |
...
- Historical payroll run/payslip import, leave-balance import, KMD history import, payment history import, cost center/product category/warehouse/product master/stock import, and fixed-asset import are now available through API and CLI, but broader incumbent-system cutover is still incomplete.
...
1. Extend historical migration beyond payroll, tax history, cost-center/product-category/warehouse/product/stock, and fixed assets into broader incumbent-system cutover imports.
