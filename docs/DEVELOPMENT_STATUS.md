...
| Area | Status | Notes |
|------|--------|-------|
| Core ledger (accounts, journal entries, journal entry templates, trial balance, balance sheet, income statement) | `Working` | Core accounting paths exist and are covered by passing backend tests, including reusable balanced journal entry templates that can generate draft or posted entries. |
