...
| Multi-tenant auth, RBAC, tenant isolation | `Working` | Core tenant model is in place; API startup now rejects missing or short production JWT secrets and missing production CORS origins, JWT access/refresh tokens are purpose-separated, refresh tokens are persisted as revocable single-use sessions with API/CLI listing and revocation, password reset uses one-time expiring tokens with request throttling and refresh-session revocation, auth security audit events cover login, logout, password, and session actions with API/CLI access for the affected user, tenant admins can inspect and revoke member refresh sessions with auth and tenant audit trails, tenant audit events cover organization settings changes, tenant user administration blocks owner-role assignment plus self role changes/removal, and tenant user/invitation administration plus audit events now have API/CLI coverage and settings UI visibility. Broader auth administration polish is still needed. |
...
1. Extend historical migration beyond payroll, tax history, quotes/orders/recurring templates, cost-center/product-category/warehouse/product/stock, and fixed assets into broader incumbent-system cutover imports, building on the migration bundle preflight validator.
