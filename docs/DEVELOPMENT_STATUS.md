...
| Report exports | `Beta` | Trial balance, balance sheet, income statement, and cash flow now have backend CSV export through API and CLI. XLSX and extended report export coverage are still incomplete. |
| Cash flow reporting | `Beta` | Present in code and UI with API/service validation for malformed and inverted periods, but needs more accountant-grade classification checks before stronger claims. |
