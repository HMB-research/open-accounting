...
| Invoice, journal-entry, payment, bank-transaction, asset, and year-end close document attachments | `Beta` | Files can be uploaded, listed, downloaded, deleted, reviewed, approved, and rejected for core accounting records, bank reconciliation evidence, fixed assets, and year-end close packs. Document type, retention date, explicit review/approval/rejection decisions, review notes, tenant-wide review queues with a dedicated UI, tenant-wide retention review with a dedicated UI, explicit evidence-policy evaluation, reconciliation completion enforcement, and close-pack enforcement now exist. Broader workflow-level policy enforcement is still incomplete. |
...
2. Extend the new accountant portfolio rollup into deeper exception actions beyond banking, including dedicated accounting follow-up workflows.
3. Add more automated workflow blocks on top of document evidence-policy evaluation beyond reconciliation and year-end close packs.
