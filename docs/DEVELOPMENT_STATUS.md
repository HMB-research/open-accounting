...
| Multi-tenant auth, RBAC, tenant isolation | `Working` | Core tenant model is in place; API startup now rejects missing or short production JWT secrets and missing production CORS origins. Stronger session management and broader auth hardening are still needed. |
...
| Invoice, journal-entry, payment, bank-transaction, and asset document attachments | `Beta` | Files can be uploaded, listed, downloaded, and deleted for core accounting records, bank reconciliation evidence, and fixed assets. Document type, retention date, explicit review/approval/rejection decisions, review notes, and a tenant-wide retention review queue now exist, but policy automation is still missing. |
