...
| Area | Status | Notes |
|------|--------|-------|
| Core ledger (accounts, journal entries, journal entry templates, recurring journal entry generation, trial balance, balance sheet, income statement) | `Working` | Core accounting paths exist and are covered by passing backend tests, including grouped chart-of-accounts hierarchy views, reusable balanced journal entry templates that can generate draft or posted entries, and recurring templates that can be generated one-by-one or in due batches through API/CLI. |
