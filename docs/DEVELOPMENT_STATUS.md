...
| Cash flow reporting | `Beta` | Present in code and UI with API/service validation for malformed and inverted periods plus default-chart and name-based custom-chart classification checks for cash/bank, receivables, payables, payroll, taxes, interest, loans, share capital, dividends, and fixed assets. Indirect-method reporting and configurable mapping rules are still incomplete. |
...
| Close/reopen workflow with audit trail | `Beta` | Explicit close and reopen actions exist in API, CLI, and company settings, with history, operator notes, auditable fiscal-year reviewer sign-off plus approved close-pack evidence on close, a safety block against reopening a year-end after carry-forward has been posted, and an explicit carry-forward reversal step for controlled corrections. |
| Fiscal year close checklist and carry-forward workflow | `Beta` | API, CLI, and company settings expose year-end readiness, year-end close packs with trial balance/balance sheet/income statement, retained-earnings mapping, reviewer sign-off before fiscal-year close, approved close-pack evidence enforcement before fiscal-year close and carry-forward, explicit carry-forward posting after year-end lock, and dated carry-forward reversal. UI polish around evidence collection is still thin. |
| Invoice, journal-entry, payment, bank-transaction, asset, and year-end close document attachments | `Beta` | Files can be uploaded, listed, downloaded, and deleted for core accounting records, bank reconciliation evidence, fixed assets, and year-end close packs. Document type, retention date, explicit review/approval/rejection decisions, review notes, tenant-wide retention review, explicit evidence-policy evaluation, reconciliation completion enforcement, and close-pack enforcement now exist. Broader workflow-level policy enforcement is still incomplete. |
...
| Inventory and warehouse flows | `Beta` | Product/category/warehouse CRUD, CSV imports, signed stock adjustments, stock import, per-warehouse stock levels, warehouse transfers, and reserve/release stock allocations exist through API and CLI. Fulfillment allocation and deeper inventory costing workflows remain thin. |
...
2. Extend close-pack evidence collection and review ergonomics in the UI on top of the enforced year-end pack, close/reopen, and carry-forward foundation.
...
4. Add more automated workflow blocks on top of document evidence-policy evaluation beyond reconciliation and year-end close packs.
5. Add broader auth administration controls and remaining auth hardening.
6. Add deployment-specific offsite storage and scheduled restore-drill runbooks around the backup and restore scripts.
