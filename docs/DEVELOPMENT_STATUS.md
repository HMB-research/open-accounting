...
| CLI and API token automation | `Working` | `cmd/oa` supports operational health/demo checks, auth registration/login/refresh/token bootstrap, tenant lifecycle/user/invitation administration, plugin and plugin-registry administration, token management, accounts, contacts, employees, payroll run lifecycle, payroll history, leave balances and leave records, TSD/KMD declarations and exports, invoices, payments, payment reminders and reminder rules, email settings/templates/logs/send actions, interest settings/calculations/history, period close/year-end workflows, banking accounts, bank transaction import/matching/review/reconciliation, quotes, orders, recurring invoices, fixed assets, inventory and warehouses, cost centers, analytics, core reports, document upload/download/review-summary workflows, and opening-balance imports using tenant-scoped API tokens. |
...
| Close/reopen workflow with audit trail | `Beta` | Explicit close and reopen actions exist in API, CLI, and company settings, with history, operator notes, and a safety block against reopening a year-end after carry-forward has been posted. |
| Fiscal year close checklist and carry-forward workflow | `Beta` | API, CLI, and company settings expose year-end readiness, retained-earnings mapping, and an explicit carry-forward journal step after year-end lock. Reopen reversal and deeper year-end packs are still missing. |
...
| Plugin marketplace | `Beta` | Registry, install, permission, tenant enablement, and settings flows exist. Backend hook/route runtime execution is not implemented and is now rejected explicitly instead of silently no-oping; frontend slots remain metadata/placeholders. |
