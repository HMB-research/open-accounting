...
| CLI and API token automation | `Working` | `cmd/oa` supports token bootstrap, token management, accounts, contacts, employees, payroll history, leave balances, TSD/KMD declarations and exports, invoices, core reports, document workflows, and opening-balance imports using tenant-scoped API tokens. |
