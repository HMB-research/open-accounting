...
When the related files are present in the same bundle, the validator also checks references such as commercial documents and e-invoice suppliers to contacts, payments to CSV or XML invoices, payroll/leave/TSD rows to employees, expense account/payment account codes to accounts, products to product categories, stock rows to products and warehouses, cost centers to parent cost centers, cost allocations to cost centers, product categories to parent categories, and opening balances or journals to accounts.
