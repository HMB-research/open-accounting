...
Validates the same bundle, builds the same execution plan, and returns a `summary`, `plan`, `steps`, and `remediation_actions` run object. With `confirm: false`, the endpoint is non-mutating and returns planned steps with `needs_confirmation`. With `confirm: true`, all plan steps must be `READY`; otherwise the response is `409` with a blocked run and missing-context or remediation details. Confirmed ready runs execute the ordered imports server-side through the existing tenant-scoped import services, mark each step `SUCCEEDED` or `FAILED`, and return `400` with the failed run when any import fails. Passing `resume_from_run` with a prior execution response marks matching previously `SUCCEEDED` steps as already complete, preserves their response payloads, sets `resumed` and `resumed_step_count`, and imports only the remaining planned steps. This endpoint is guarded, resume-aware execution for ready bundles; durable dashboard-side run storage remains a separate production-hardening track.
