...
Required CSV columns are `name`, `purchase_date`, and `purchase_cost`. Optional columns include `asset_number`, `category_id`, `category_name`, `status`, `description`, `supplier_id`, `supplier_code`, `invoice_id`, serial/location fields, depreciation settings, `accumulated_depreciation`, `book_value`, `last_depreciation_date`, disposal fields, and asset/depreciation account IDs or account codes. ID columns must be valid UUIDs; account-code columns are `asset_account_code`, `depreciation_expense_account_code`, and `accumulated_depreciation_account_code`. `supplier_code` resolves through contact codes before storing the resolved supplier UUID, and invalid or missing supplier codes are returned as row-level errors. Omitted asset numbers are generated; supplied asset numbers are preserved and checked for duplicates.
