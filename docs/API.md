...
Supported header aliases include `id` / `contact_id`, `name` / `company_name`, `type`, `payment_terms_days` / `payment_days`, and standard contact metadata such as `email`, `phone`, `reg_code`, and `vat_number`. Optional imported IDs must be valid UUIDs and are preserved so cutover files can refer to contacts through `contact_id` or `supplier_id`.
...
Imports incumbent-system KMD history without overwriting existing declaration periods. Required CSV columns are `year`, `month`, and `row_code`; each row must include `tax_base` or `tax_amount`. Optional `status`, `submitted_at`, `description`, `total_output_vat`, and `total_input_vat` fields must use importer-compatible values, and period-level status/submission/VAT-total values must stay consistent across rows in the same declaration period.
