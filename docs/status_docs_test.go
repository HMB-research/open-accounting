...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"`cd frontend && bun run test:prepared` passes with 30 files and 557 tests",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"remediation data available through API/CLI for workspace integration with workspace queues, stable assignment keys, priorities, and due windows",
