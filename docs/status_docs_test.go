...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"`go test ./internal/cutover ./cmd/api ./cmd/oa -count=1` and `cd frontend && bun run test:prepared -- MigrationWorkbench.test.ts api.test.ts` pass focused migration progress telemetry coverage",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"Accountant-workspace launch handoff, broader vendor mapping presets, deeper cross-file validation, and richer step-duration/streaming telemetry remain open.",
