...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"grouped migration remediation actions for ready bundles, unsupported file kinds, missing columns, missing references, duplicate identifiers, grouped consistency failures, malformed IDs, invalid row values, and warning review",
		...
		"Full incumbent-system cutover paths, broader vendor mapping presets, deeper cross-file validation, and workspace assignment of cutover tasks remain open.",
