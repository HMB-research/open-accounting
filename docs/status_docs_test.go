...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		"README.md": {
			...
			"Broader plugin production hardening beyond the current loopback HTTP runtime, supervised package runtime startup/proxy/shutdown/status/manual restart, and safe operator-bundled frontend component registry",
		},
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"Automatic restart policy, OS-level sandboxing, and broader production plugin isolation",
			"KMD tax remediation actions stage",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"loopback-only out-of-process HTTP backend runtime and supervised package runtime startup/proxy/shutdown for manifest-declared hooks and tenant-scoped plugin routes, package runtime lifecycle/health/crash/backoff status plus manual restart through API/CLI",
		"safe operator-bundled frontend component registration",
		"Automatic restart policy, OS-level sandboxing, and broader production plugin isolation remain incomplete.",
