...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		"README.md": {
			...
			"Broader plugin production hardening beyond the current loopback HTTP runtime, supervised package runtime startup/proxy/shutdown, and safe operator-bundled frontend component registry",
		},
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"Restart/backoff management, OS-level sandboxing, and broader production plugin isolation",
			"KMD tax remediation actions stage",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"Restart/backoff management, OS-level sandboxing, and broader production plugin isolation remain incomplete.",
