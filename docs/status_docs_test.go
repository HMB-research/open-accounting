...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"dependency-aware execution plans for ready bundles with API/CLI import steps and missing-context markers for bank-transaction and opening-balance imports",
		...
		"Full incumbent-system mutating cutover orchestration, broader vendor mapping presets, deeper cross-file validation, and dashboard execution of cutover tasks remain open.",
