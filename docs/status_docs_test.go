...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"dependency-aware execution plans for ready bundles with API/CLI import steps, missing-context markers for bank-transaction and opening-balance imports, and guarded CLI execution for fully ready plans",
		...
		"Durable server-side/dashboard incumbent-system mutating cutover orchestration, broader vendor mapping presets, and deeper cross-file validation remain open.",
