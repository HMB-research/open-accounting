...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"`cd frontend && bun run test:prepared` passes with 31 files and 559 tests",
			"`cd frontend && bun run test:prepared -- AccountantReviewPanel.test.ts AccountantPortfolioPanel.test.ts review-workspace.test.ts` passes targeted accountant workspace assignment queue coverage",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"a dashboard assignment queue that aggregates close, banking, document-retention, payroll-run, TSD, and KMD remediation actions with tenant-scoped deep links plus CLI commands",
