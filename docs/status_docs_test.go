...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"`cd frontend && bun run test:prepared` passes with 31 files and 563 tests",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"pending-document assignment approval, calculated payroll-run approval, approved payroll-run TSD generation, draft expense submission, submitted expense approval, and approved-expense ledger posting directly from the dashboard",
