...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"invoice `amount_paid` consistency against imported invoice CSV totals and statuses, payment allocation totals against imported invoice CSV and e-invoice XML totals, payment allocation currency consistency against imported invoice CSV and e-invoice XML currencies, and ambiguous invoice-number reference checks",
