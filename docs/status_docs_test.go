...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	activeDocs := map[string]string{
		...
		"docs/DEPLOYMENT.md":                                  read("DEPLOYMENT.md"),
	...
	required := map[string][]string{
		"README.md": {
			...
			"Detailed feature status and validation evidence live in [Development Status](docs/DEVELOPMENT_STATUS.md)",
			"use-case-level coverage lives in [Use Case Coverage](docs/USE_CASE_COVERAGE.md)",
			"CLI%20coverage-100%25-brightgreen",
			"docs%20gate-passing-brightgreen",
			"demo%20E2E-blocking%20CI-brightgreen",
			"Local seeded smoke plus full demo E2E shards are CI gates",
			"Production hardening, deeper historical cutover tooling, broader accountant-workspace execution",
		},
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"focused KMD VAT-history support reconciliation and historical-journal VAT metadata import coverage",
			...
			"further provider-specific mapping depth, cross-file validation outside payroll/TSD history and KMD VAT support, and other dashboard-side incumbent-system mutating cutover controls are still incomplete",
			...
			"KMD migration preflight now rejects declared output/input VAT totals that do not match same-bundle invoice, e-invoice, and VAT-rated historical journal support totals",
		...
		"docs/CURRENT_PRODUCT_LIMITS.md": {
			...
			"KMD VAT-total reconciliation against same-bundle invoice/e-invoice/journal VAT support",
	...
	readmeLedgerSnippets := []string{
		"Full local baseline last verified on",
		"The follow-up ",
		"locally revalidated with focused",
		"PR #62 CI run",
	}
	for _, snippet := range readmeLedgerSnippets {
		if strings.Contains(activeDocs["README.md"], snippet) {
			t.Fatalf("README.md should summarize current status instead of carrying per-stage evidence ledger snippet %q", snippet)
		}
	}
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"payment bank-account and provider journal-line/cost-allocation cross-reference tests, provider opening-balance amount alias tests, provider historical-journal import alias tests",
		"focused KMD external VAT support migration tests",
		"Further provider-specific mapping depth, cross-file validation outside payroll/TSD history and KMD VAT support, and dashboard-side mutating cutover controls remain open.",
