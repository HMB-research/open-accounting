...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		"README.md": {
			...
			"PR #62 CI run `27510435008` at commit `76b1a58`",
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"tax-report assignment execution stage",
			"plugin HTTP runtime stage",
			"loopback-only out-of-process HTTP backend runtime for manifest-declared hooks and tenant-scoped plugin routes",
			"dynamic plugin Svelte component loading",
			"runtime process supervision/packaging",
			"KMD tax remediation actions stage",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"plugin HTTP runtime stage",
		"loopback-only out-of-process HTTP backend runtime for manifest-declared hooks and tenant-scoped plugin routes",
		"Dynamic frontend component loading, runtime process supervision/packaging, and broader production sandboxing remain incomplete.",
