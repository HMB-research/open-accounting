...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"pending-document assignment approval, document-retention date setting, calculated payroll-run payment-date setting and approval, approved payroll-run TSD generation, KMD regeneration/XML export assignment actions, draft expense submission, submitted expense approval, approved-expense ledger posting, fiscal-year close, and carry-forward posting directly from the dashboard",
		...
		"focused accountant review-panel document-retention execution tests",
