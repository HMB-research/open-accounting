...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"`cd frontend && bun run test:prepared` passes with 31 files and 567 tests",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"dependency-aware execution plans for ready bundles with API/CLI import steps, missing-context markers for bank-transaction and opening-balance imports, and guarded CLI plus server-side API execution for fully ready plans",
		...
		"Durable resumable dashboard incumbent-system mutating cutover orchestration, broader vendor mapping presets, and deeper cross-file validation remain open.",
