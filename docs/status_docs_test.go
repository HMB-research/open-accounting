...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		"README.md": {
			...
			"PR #62 CI run `27504200828` at commit `588efdd`",
			...
			"approved tax/support documents before declarations can be marked submitted",
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"further provider-specific mapping depth and other dashboard-side incumbent-system mutating cutover controls are still incomplete",
			...
			"approved tax/support evidence blockers before marking TSD declarations submitted",
			"TSD/KMD declaration tax-support attachments, and TSD/KMD submission evidence enforcement",
...
func TestCLIGuideDocumentsPayrollMigrationSamples(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"`tsd mark-submitted` requires one approved `tax_support` or `supporting_document` uploaded to `--entity-type tsd_declaration`",
		...
		"`tax kmd mark-submitted` requires one approved `tax_support` or `supporting_document` uploaded to `--entity-type kmd_declaration`",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"focused accountant review-panel document-retention, evidence-upload, and evidence-policy approval execution tests",
		...
		"TSD submission evidence blockers requiring approved tax/support documents before marking submitted",
		"focused TSD submission evidence handler/document tests",
		"workflow blockers for reconciliation, assets, purchase invoices, journal entries, payments, expenses, leave records, TSD declarations, KMD declarations, close packs, and TSD/KMD submission",
		"focused document entity, TSD submission evidence, and KMD submission evidence tests",
