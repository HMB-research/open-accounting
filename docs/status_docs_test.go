...
func TestStatusDocumentationTracksCurrentGates(t *testing.T) {
	...
	required := map[string][]string{
		...
		"docs/DEVELOPMENT_STATUS.md": {
			...
			"`cd frontend && bun run test:prepared` passes with 31 files and 561 tests",
			"`cd frontend && bun run test:prepared -- AccountantReviewPanel.test.ts AccountantPortfolioPanel.test.ts review-workspace.test.ts api.test.ts` passes targeted accountant workspace assignment queue, expense frontend follow-up, document assignment approval, payroll assignment approval, and payroll TSD assignment generation coverage",
...
func TestUseCaseCoverageMatrixDocumentsGoalEvidence(t *testing.T) {
	...
	for _, snippet := range []string{
		...
		"pending-document assignment approval, calculated payroll-run approval, and approved payroll-run TSD generation directly from the dashboard",
