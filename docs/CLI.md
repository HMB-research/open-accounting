...
Product category CSV imports require `name`, can preserve optional UUIDs from `id`, `category_id`, or `product_category_id`, and can resolve parents from `parent_id`, `parent_category_id`, or `parent_name`; `migration validate` treats parent/category ID columns as UUID/preserved-ID references and name columns as name references, and also rejects blank category names plus same-file `parent_name` references that point to later rows. Product CSV imports require `name` and `sales_price`; optional columns include `code`, `product_type`, `category_id`, `category_name`, prices, VAT, reorder settings, account IDs, account-code columns `sale_account_code`, `purchase_account_code`, and `inventory_account_code`, inventory tracking, status, barcode, `supplier_id`, `supplier_code`, and lead time. Product imports generate new UUIDs rather than preserving `id` or `product_id` values; supplied product codes are preserved and are the same-bundle lookup key for product lines and stock. Product `category_id`, direct account ID, and `supplier_id` values must be valid UUIDs for existing or already imported records; `supplier_code` values resolve through contact codes before storing the resolved supplier UUID. Invalid or missing category IDs and supplier codes are returned as row-level import errors. Product import JSON includes row-level errors when rows are skipped.

Fixed-asset CSV imports require `name`, `purchase_date`, and `purchase_cost`; optional columns include `asset_number`, category ID/name, status, description, `supplier_id`, `supplier_code`, `invoice_id`, serial/location fields, depreciation settings, accumulated depreciation, book value, disposal fields, and asset/depreciation account IDs or account-code columns. Direct `supplier_id`, `invoice_id`, category, and account ID values must be valid UUIDs for existing or already imported records; `supplier_code` values resolve through contact codes before storing the resolved supplier UUID. Invalid or missing supplier codes are returned as row-level import errors.
