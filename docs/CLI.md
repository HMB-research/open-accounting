...
Every report command supports `--json` for automation. Core statement commands also support backend CSV export with `--csv`; omit `--output` to stream CSV to stdout.
