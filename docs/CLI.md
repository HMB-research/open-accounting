...
`tax kmd mark-submitted` requires one approved `tax_support` or `supporting_document` uploaded to `--entity-type kmd_declaration` with the declaration ID as `--entity-id`; missing or pending evidence is returned as a conflict and the declaration remains draft. `tax kmd mark-submitted` records the declaration as submitted with the current server timestamp. `tax kmd mark-accepted` records e-MTA acceptance for a submitted KMD period. Both commands return a status payload in JSON mode and power the accountant workspace KMD acceptance assignment action.
