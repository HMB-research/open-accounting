...
Payment, order, and fixed-asset preflight also rejects non-UUID values in payment `invoice_id`, order `quote_id`, fixed-asset `category_id`, and fixed-asset `invoice_id` before any mutating import runs. Fixed-asset source invoices included in the same bundle must be purchase invoices, supplier identity fields on the asset must match the source invoice contact when both rows provide the same field, and the asset purchase-cost total must not exceed the imported source invoice total. Use invoice numbers, product/warehouse/cost-center/supplier identity columns, or provider aliases where the importer documents lookup columns.
