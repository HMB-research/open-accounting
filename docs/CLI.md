...
go run ./cmd/oa ops backup schedule-systemd --output-dir ./deploy/systemd --scripts-dir /opt/open-accounting/scripts --env-file /etc/open-accounting/backup.env --offsite-provider s3 --systemd-unit-dir /etc/systemd/system --dry-run
```

`ops backup create` delegates to `db-backup.sh`, which requires `DATABASE_URL` unless `--database-url` is passed. `ops backup restore-drill` requires a separate disposable restore database, refuses to restore into the source URL when `DATABASE_URL` or `--source-url` matches, and forwards `--status-file` so scheduled drills can publish Prometheus textfile metrics and structured failure codes. `ops backup offsite-sync` requires exactly one destination: `--s3-uri` or `--rclone-remote`. `ops backup schedule-systemd` writes systemd service/timer templates for backup, offsite sync, health checks, and weekly restore drills while keeping live credentials in an external `EnvironmentFile`; `--offsite-provider s3|rclone` selects the generated credential template, and `--systemd-unit-dir` is embedded in the generated install helper that copies units, preserves existing secrets, reloads systemd, and enables the four timers.
...
Stock-adjustment preflight rejects same-bundle `product_code` rows that point at service products or products with `track_inventory=false`; stock imports must target tracked `GOODS` products before mutating execution can run.
