...
Budget periods are `MONTHLY`, `QUARTERLY`, and `ANNUAL`. Cost center CSV imports require `code` and `name`, with optional `parent_code`, `budget_amount`, `budget_period`, `status`, and `is_active`; imports generate new UUIDs and preserve codes for downstream lookup. Cost allocations assign positive journal-entry-line amounts to cost centers and can be filtered by cost center, journal entry line, and allocation date range. Cost allocation CSV imports require `journal_entry_line_id`, `amount`, `allocation_date`, and either an existing UUID in `cost_center_id` or a resolvable `cost_center_code`; optional columns include `allocation_percentage` and `notes`. Cost center reports support `--csv`, `--xlsx`, `--pdf`, and `--output`. Use `--json` on cost-center read, mutation, and import commands for automation.
