...
Payment bank-account preflight treats omitted payment `currency` values as EUR and rejects same-bundle `bank_account` rows that target a non-EUR bank account unless the payment row supplies the matching currency.
