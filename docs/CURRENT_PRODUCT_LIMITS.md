...
| --- | --- | --- |
| Historical migration and cutover | Many CSV/XML imports, provider presets, validation, saved runs, live progress, guarded API/CLI execution, and dashboard workbench flows exist. | Deeper provider-specific mapping, more cross-file validation, and more dashboard-side mutating cutover controls. A concrete known gap is KMD history preflight: declared KMD VAT totals are not yet reconciled against same-bundle invoice/e-invoice/journal VAT support. |
