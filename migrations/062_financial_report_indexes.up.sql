CREATE OR REPLACE FUNCTION add_financial_report_indexes(schema_name TEXT)
RETURNS void AS $$
BEGIN
    IF to_regclass(format('%I.journal_entries', schema_name)) IS NOT NULL THEN
        EXECUTE format('
            CREATE INDEX IF NOT EXISTS idx_%s_je_report_posted_date
            ON %I.journal_entries(tenant_id, entry_date, id)
            WHERE status = ''POSTED''
        ', replace(schema_name, '-', '_'), schema_name);
    END IF;

    IF to_regclass(format('%I.journal_entry_lines', schema_name)) IS NOT NULL THEN
        EXECUTE format('
            CREATE INDEX IF NOT EXISTS idx_%s_jel_report_entry_account
            ON %I.journal_entry_lines(tenant_id, journal_entry_id, account_id)
            INCLUDE (debit_amount, credit_amount)
        ', replace(schema_name, '-', '_'), schema_name);

        EXECUTE format('
            CREATE INDEX IF NOT EXISTS idx_%s_jel_report_account_entry
            ON %I.journal_entry_lines(tenant_id, account_id, journal_entry_id)
            INCLUDE (debit_amount, credit_amount)
        ', replace(schema_name, '-', '_'), schema_name);
    END IF;
END;
...
BEGIN
    ...
    LOOP
        PERFORM add_financial_report_indexes(tenant_schema);
    END LOOP;
END $$;
