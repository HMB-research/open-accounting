-- Migration 036: Recurring journal entry template schedules

CREATE OR REPLACE FUNCTION add_journal_entry_template_recurrence(schema_name TEXT) RETURNS VOID AS $$
BEGIN
    EXECUTE format('
    ...
    EXECUTE format('
        CREATE INDEX IF NOT EXISTS idx_%s_jet_next_generation
        ON %I.journal_entry_templates(tenant_id, next_generation_date)
...
BEGIN
    ...
    LOOP
        PERFORM add_journal_entry_template_recurrence(tenant_schema);
    END LOOP;
END $$;
