-- Rollback migration 047: Remove document evidence support from leave records
...
BEGIN
    ...
    LOOP
        EXECUTE format('DELETE FROM %I.documents WHERE entity_type = ''leave_record''', tenant_schema);
...
DROP FUNCTION IF EXISTS add_leave_record_document_entity(TEXT);
