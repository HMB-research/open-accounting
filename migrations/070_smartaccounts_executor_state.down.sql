-- Restore the migration-069 tenant bootstrap before removing 070 structures.
...
DO $$ DECLARE tenant_schema TEXT; BEGIN
  FOR tenant_schema IN SELECT schema_name FROM tenants WHERE schema_name IS NOT NULL LOOP
    EXECUTE format('DROP TABLE IF EXISTS %I.smartaccounts_correction_reviews', tenant_schema);
    EXECUTE format('DROP TABLE IF EXISTS %I.smartaccounts_financial_postings', tenant_schema);
    EXECUTE format('DROP TABLE IF EXISTS %I.smartaccounts_source_account_mappings', tenant_schema);
    EXECUTE format('DROP TABLE IF EXISTS %I.smartaccounts_executor_previews', tenant_schema);
  END LOOP;
END $$;

DROP FUNCTION IF EXISTS add_smartaccounts_executor_tables(TEXT);
