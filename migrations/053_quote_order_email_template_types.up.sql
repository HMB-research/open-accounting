-- Migration 053: allow quote and order delivery email templates.
--
-- Quote/order delivery adds persisted templates with template_type values
-- QUOTE_SEND and ORDER_CONFIRM. Older tenant schemas still have the check
-- constraint from the original email/reminder migrations, so normalize the
-- allowed values for existing schemas and future tenant bootstrap.
