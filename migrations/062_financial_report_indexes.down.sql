...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I.%I', tenant_schema, format('idx_%s_je_report_posted_date', replace(tenant_schema, '-', '_')));
        EXECUTE format('DROP INDEX IF EXISTS %I.%I', tenant_schema, format('idx_%s_jel_report_entry_account', replace(tenant_schema, '-', '_')));
        EXECUTE format('DROP INDEX IF EXISTS %I.%I', tenant_schema, format('idx_%s_jel_report_account_entry', replace(tenant_schema, '-', '_')));
    END LOOP;
END $$;
...
DROP FUNCTION IF EXISTS add_financial_report_indexes(TEXT);
