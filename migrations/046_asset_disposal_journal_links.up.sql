-- Migration 046: Link fixed-asset disposals to posted journal entries

CREATE OR REPLACE FUNCTION add_fixed_asset_disposal_journal_links(schema_name TEXT) RETURNS VOID AS $$
BEGIN
    BEGIN
        EXECUTE format(
            'ALTER TABLE %I.fixed_assets
                ADD COLUMN IF NOT EXISTS disposal_journal_entry_id UUID REFERENCES %I.journal_entries(id)',
            schema_name,
            schema_name
        );

        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS %I ON %I.fixed_assets(disposal_journal_entry_id)
                WHERE disposal_journal_entry_id IS NOT NULL',
            'idx_' || replace(schema_name, '-', '_') || '_fassets_disposal_je',
            schema_name
        );
...
BEGIN
    ...
    LOOP
        PERFORM add_fixed_asset_disposal_journal_links(tenant_schema);
    END LOOP;
END $$;
