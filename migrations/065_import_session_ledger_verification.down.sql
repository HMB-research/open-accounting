-- Roll back the ledger-verification receipt metadata while preserving the
-- receipt-only import-session schema introduced by deployed migration 064.
...
BEGIN
    ...
    LOOP
        IF to_regclass(format('%I.import_sessions', tenant_schema)) IS NOT NULL THEN
            EXECUTE format(
                'ALTER TABLE %I.import_sessions DROP COLUMN IF EXISTS ledger_verification',
                tenant_schema
            );
        END IF;
    END LOOP;
...
-- Restore the migration-064 tenant bootstrap exactly: import-session tables
-- are created, but the forward-only ledger-verification upgrade is not run.
