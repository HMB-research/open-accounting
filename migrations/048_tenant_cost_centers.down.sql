-- Rollback migration 048: Remove tenant-scoped cost center tables
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP TABLE IF EXISTS %I.cost_allocations', tenant_schema);
        EXECUTE format('DROP TABLE IF EXISTS %I.cost_centers', tenant_schema);
    END LOOP;
END $$;

DROP FUNCTION IF EXISTS add_cost_center_tables(TEXT);
