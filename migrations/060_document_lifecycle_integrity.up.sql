-- Migration 060: Add document lifecycle integrity constraints

CREATE OR REPLACE FUNCTION add_document_lifecycle_integrity(schema_name TEXT) RETURNS VOID AS $$
BEGIN
    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_superseded_by_document_fk', schema_name);
    EXECUTE format('ALTER TABLE %I.documents ADD CONSTRAINT documents_superseded_by_document_fk FOREIGN KEY (superseded_by_document_id) REFERENCES %I.documents(id) ON DELETE RESTRICT NOT VALID', schema_name, schema_name);

    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_reviewed_by_user_fk', schema_name);
    EXECUTE format('ALTER TABLE %I.documents ADD CONSTRAINT documents_reviewed_by_user_fk FOREIGN KEY (reviewed_by) REFERENCES public.users(id) ON DELETE SET NULL NOT VALID', schema_name);

    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_lifecycle_actioned_by_user_fk', schema_name);
    EXECUTE format('ALTER TABLE %I.documents ADD CONSTRAINT documents_lifecycle_actioned_by_user_fk FOREIGN KEY (lifecycle_actioned_by) REFERENCES public.users(id) ON DELETE SET NULL NOT VALID', schema_name);

    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_legal_hold_actioned_by_user_fk', schema_name);
    EXECUTE format('ALTER TABLE %I.documents ADD CONSTRAINT documents_legal_hold_actioned_by_user_fk FOREIGN KEY (legal_hold_actioned_by) REFERENCES public.users(id) ON DELETE SET NULL NOT VALID', schema_name);

    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_superseded_link_check', schema_name);
    EXECUTE format(
        'ALTER TABLE %I.documents ADD CONSTRAINT documents_superseded_link_check CHECK ((lifecycle_status = ''SUPERSEDED'' AND superseded_by_document_id IS NOT NULL) OR (lifecycle_status <> ''SUPERSEDED'' AND superseded_by_document_id IS NULL)) NOT VALID',
        schema_name
    );

    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_lifecycle_actioned_at_check', schema_name);
    EXECUTE format(
        'ALTER TABLE %I.documents ADD CONSTRAINT documents_lifecycle_actioned_at_check CHECK (lifecycle_status = ''ACTIVE'' OR lifecycle_actioned_at IS NOT NULL) NOT VALID',
        schema_name
    );

    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_legal_hold_audit_check', schema_name);
    EXECUTE format(
        'ALTER TABLE %I.documents ADD CONSTRAINT documents_legal_hold_audit_check CHECK (legal_hold = FALSE OR (legal_hold_note IS NOT NULL AND btrim(legal_hold_note) <> '''' AND legal_hold_actioned_at IS NOT NULL)) NOT VALID',
        schema_name
    );
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    LOOP
        PERFORM add_document_lifecycle_integrity(tenant_schema);
    END LOOP;
END $$;
