-- Migration 047: Allow document evidence on leave records

CREATE OR REPLACE FUNCTION add_leave_record_document_entity(schema_name TEXT) RETURNS VOID AS $$
BEGIN
    EXECUTE format('ALTER TABLE %I.documents DROP CONSTRAINT IF EXISTS documents_entity_type_check', schema_name);
    EXECUTE format(
        'ALTER TABLE %I.documents ADD CONSTRAINT documents_entity_type_check CHECK (entity_type IN (''invoice'', ''journal_entry'', ''payment'', ''bank_transaction'', ''asset'', ''expense'', ''quote'', ''order'', ''year_end_close'', ''leave_record''))',
        schema_name
    );
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    LOOP
        PERFORM add_leave_record_document_entity(tenant_schema);
    END LOOP;
END $$;
