-- Migration 045 down: Remove inventory movement tracking metadata
    ...
    LOOP
        BEGIN
            EXECUTE format(
                ...
                tenant_schema,
                'idx_' || replace(tenant_schema, '-', '_') || '_inv_mov_lot'
            );
            EXECUTE format(
                'DROP INDEX IF EXISTS %I.%I',
                tenant_schema,
                'idx_' || replace(tenant_schema, '-', '_') || '_inv_mov_serial'
            );
            EXECUTE format(
...
DROP FUNCTION IF EXISTS add_inventory_movement_tracking_metadata(TEXT);
